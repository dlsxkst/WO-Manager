 ...
 <div class="col-sm-12">
    <div class="card card-primary card-outline card-outline-tabs">
      <div class="card-body">
        ...
        $jml = $this->db->query("SELECT * FROM `tb_pesan` WHERE id_penerima='$id_vendor' AND pengirim='tb_wo'")->num_rows();
         ?>
         <?php if ($jml == 0) {
        ...
        <ul class="contacts-list">
          <?php
            $id_vendor = $this->session->userdata('id_vendor');
            $sql_pelanggan = $this->db->query("SELECT * FROM `tb_wo`");
            foreach ($sql_pelanggan->result_array() as $qp) {
              $jumpes = $this->db->query("SELECT * FROM `tb_pesan` WHERE id_pengirim='".$qp['id_wo']."' AND id_penerima='$id_vendor' AND pengirim='tb_wo'")->num_rows();
          ?>
          <?php if ($jumpes > 0) { ?>
            <li>
              <a href="<?= base_url('vendor/chat_wo?to=wo&id='.$qp['id_wo']); ?>">
                <img class="contacts-list-img" src="<?= base_url('assets/foto/'.$qp['gambar']); ?>">
                <div class="contacts-list-info">
                  <span class="contacts-list-name text-primary mt-2"><?= $qp['nama_toko']; ?><small class="contacts-list-date float-right badge badge-danger"><?= $jumpes; ?></small></span>
                </div>
              </a>
            </li>
            <?php } ?>
...
<?php } else { ?>
  <?php
    ...
    if (!empty($_GET['to']) AND 'wo' !== $_GET['to']) {
      redirect();
    } else {
      $get = 'tb_wo';
      $db_get = 'id_wo';
    ...
    $id_get = $_GET['id'];
    $back = 'vendor/chat_wo?to='.$_GET['to'].'&id='.$id_get;
    $enter = '
';
  ?>
  <div class="col-sm-12">
    <div class="card mb-4">
          ...
          <?php foreach ($sql_pesan->result_array() as $qp) { ?>
            <?php if ($get == $qp['pengirim'] OR $get == $qp['penerima']) { ?>
              <?php if ($id_get == $qp['id_pengirim'] OR $id_get == $qp['id_penerima']) { ?>
                ...
                <?php } else { ?>
                  <?php if ('tb_wo' == $qp['pengirim'] AND 'tb_vendor' == $qp['penerima']) { ?>
      ...
      <div class="card-footer">
        <form action="<?= base_url('vendor/kirim_pesan_wo'); ?>" method="post">
